         ...
         try:
              ...
              try:
                   ...
                   try:
                        ...
                        try:
                             iso_date = datetime.datetime.strptime(date_str, "%d %b %Y").strftime("%Y-%m-%d") #DD MMM YYYY to YYYY-MM-DD
                        except ValueError:
                             try:
                                  iso_date = datetime.datetime.strptime(date_str, "%d %b %y").strftime("%Y-%m-%d") #DD MMM YY to YYYY-MM-DD
                             except ValueError:
                                  try:
                                       iso_date = datetime.datetime.strptime(date_str, "%d-%b-%Y").strftime("%Y-%m-%d") #DD-MMM-YYYY to YYYY-MM-DD
                                  except ValueError:
                                       try:
                                            ...
                                            iso_date = "Invalid Date format : " + date_str
    return(iso_date)
#===================================================================================================
...
f.write(100*'=')
file_name_wc = glob.glob('C:/Users/MRSD/Downloads/OpTransactionHistory*.xls') # Filename pattern search using glob
...
for row_num in range(0,num_rows):
    ...
    first_cell = first_sheet.cell(row_num,2) #Column number of 1st Column in the Txn Records table
    first_cell_type = ctype_text.get(first_cell.ctype,'Unknown Type')
    first_cell_value = first_cell.value
    #print('cell(%d,%d) = (type = %s, value = %s)' % (row_num, 2, first_cell_type,first_cell_value))

    # Once header is found extract the txn records
    if header_found == 'Y':
       if first_cell_value != '': #First cell Value should not be empty
          ...
          cell_value = cell_value.replace(" '"," ") #Replace "10 Apr '18" to "10 Apr 18"
          ...
          (cell,cell_type,cell_value) = getCellDetails(row_num,8)
          debit = cell_value.strip()

          (cell,cell_type,cell_value) = getCellDetails(row_num,12)
          credit = cell_value.strip()

          (cell,cell_type,cell_value) = getCellDetails(row_num,14)
       ...
       if first_cell_value == '': #Cell value is empty
          header_found = 'E'
          first_header_line_found = 'E'
    # Find header record and set flag
    if first_header_line_found == 'Y':
       if first_cell_value == '': #Second Header
          header_found = 'Y'
          f.write("\n" + 'txn_date|txn_no|txn_desc|debit|credit|balance')
    if first_cell_value == 'Tran Date': #First Header
       first_header_line_found = 'Y'
